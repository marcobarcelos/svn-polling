...
class SvnPolling extends EventEmitter {
	constructor(options) {
		...
		this.svn = new Svn(options.repositoryPath);
	}
...
function emitDataEvent(logs) {
	...
	let maxRevision = Math.max.apply(Math, logs.map(log => log.revision));
	let minRevision = Math.min.apply(Math, logs.map(log => log.revision));
	...
	return storage.setItem(this.options.remoteUrl, maxRevision);
}
